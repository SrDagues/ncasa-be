...
public class JpaNotificationRepositoryAdapter implements NotificationRepository {
    ...
    private JpaNotificationEntity entity(Notification n){var e=new JpaNotificationEntity();e.id=n.id().value();e.eventId=n.eventId().value();e.recipientAccountId=n.recipient().value();e.householdId=n.householdId().value();e.planId=n.planId().value();e.kind=n.kind().name();e.subject=n.subject();e.amount=n.amount().amount();e.currency=n.amount().currency();e.occurrenceDate=n.occurrenceDate();e.occurrenceNumber=n.occurrenceNumber();e.totalOccurrences=n.totalOccurrences();e.attentionReason=n.attentionReason();e.occurredAt=n.occurredAt();e.createdAt=n.createdAt();e.readAt=n.readAt();return e;}
    private Notification domain(JpaNotificationEntity e){return Notification.rehydrate(new NotificationId(e.id),new IntegrationEventId(e.eventId),new AccountRef(e.recipientAccountId),new HouseholdRef(e.householdId),new PlanRef(e.planId),NotificationKind.valueOf(e.kind),e.subject,new NotificationAmount(e.amount,e.currency),e.occurrenceDate,e.occurrenceNumber,e.totalOccurrences,e.attentionReason,e.occurredAt,e.createdAt,e.readAt);}
}
