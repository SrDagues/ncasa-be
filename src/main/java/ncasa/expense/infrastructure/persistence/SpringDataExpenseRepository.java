...
interface SpringDataExpenseRepository extends JpaRepository<JpaExpenseEntity, UUID> {
    boolean existsBySourcePlanIdAndOccurrenceKey(UUID sourcePlanId, String occurrenceKey);
    ...
    @Query(value = "select e.id from JpaExpenseEntity e where e.householdId = :householdId "
            ...
            + "and (:planId is null or e.sourcePlanId = :planId) "
    ...
    Page<UUID> findPageIds(@Param("householdId") UUID householdId,
            ...
            @Param("uncategorized")boolean uncategorized,@Param("splitType")String splitType,
            @Param("source")String source,@Param("planId")UUID planId,Pageable pageable);
